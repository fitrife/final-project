...
@section('container')
    ...
    <div class="bnsp-section section-padding px-3 py-2 bg-white rounded">
        ...
        <div class="row">
          <div class="col-md-12">
            <form method="post" action="/dashboard/adsbnsp/{{ $bnsp->slugBnsp }}" class="mb-5" enctype="multipart/form-data">
                @method("put")
                @csrf
              ...
              <div class="mb-3">
                  ...
                  <select disabled class="form-select" name="training_id">
                      @foreach ($ad as $ad)
                          <option value="{{ $ad->id }}" {{ old('ad_id', $bnsp->ad_id) == $ad->id ? 'selected' : null }}>{{ $ad->name }}</option>
                      @endforeach
                  </select>
              </div>
              <div class="mb-2">
