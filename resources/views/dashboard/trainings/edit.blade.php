    ...
    <div class="program-section section-padding px-3 py-2 bg-white rounded">
        ...
        <div class="row">
          <div class="col-md-6">
            <form
              method="post"
              action="/dashboard/trainings/{{ $training->slug }}"
              class="mb-5"
            ...
            @csrf
              <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" required autofocus value="{{ old('name', $training->name) }}">
                @error('name')
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" required value="{{ old('slug', $training->slug) }}">
                    @error('slug')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
              ...
              <div class="mb-3">
                ...
                <select class="form-select" name="training_categories_id">
                    @foreach ($trainingcategories as $category)
                        <option value="{{ $category->id }}" {{ old('training_categories_id', $training->training_categories_id) == $category->id ? 'selected' : null }}>{{ $category->name }}</option>
                    @endforeach
                  {{-- @foreach ($trainingcategories as $category)
              ...
              <div class="mb-3">
                ...
                <input type="hidden" name="oldThumbnail" value="{{ $training->thumbnail }}">
                    ...
                    <img src="{{ asset('storage/' .$training->thumbnail) }}" class="thumbnail-preview img-fluid mb-3 col-sm-5 d-block">
                    @else
                    <img class="thumbnail-preview img-fluid mb-3 col-sm-5">
                    @endif
              ...
              <div class="mb-3">
                ...
                >
                <textarea id="description" type="hidden" name="description">{{ old('description', $training->description) }}</textarea>
                    @error('description')
              ...
              <div class="mb-3">
                ...
                >
                <textarea id="theory" type="hidden" name="theory">{{ old('theory', $training->theory) }}</textarea>
                    @error('theory')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
              </div>
              <div class="mb-3">
                ...
                >
                <textarea id="participant_requirement" type="hidden" name="participant_requirement">{{ old('participant_requirement', $training->participant_requirement) }}</textarea>
              ...
              <div class="mb-3">
                ...
                >
                <textarea id="goal" type="hidden" name="goal">{{ old('goal', $training->goal) }}</textarea>
                    @error('goal')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
              </div>
              <div class="mb-3">
                ...
                >
                <textarea id="method" type="hidden" name="method">{{ old('method', $training->method) }}</textarea>
                    @error('method')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
              </div>
              <div class="mb-3">
                ...
                >
                <textarea id="facility" type="hidden" name="facility">{{ old('facility', $training->facility) }}</textarea>
                    @error('facility')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
              </div>
